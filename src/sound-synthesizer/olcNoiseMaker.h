...
//using namespace std;
#include <algorithm> // was not included before

#include <Windows.h>
using namespace std; // there was errors related
	...
	static vector<wstring> Enumerate()
	{
		...
		for (int n = 0; n < nDeviceCount; n++)
			if (waveOutGetDevCaps(n, &woc, sizeof(WAVEOUTCAPS)) == S_OK)
				sDevices.push_back(s2ws(std::string(woc.szPname))); // also changed
		return sDevices; // https://github.com/OneLoneCoder/synth/issues/14
	}
