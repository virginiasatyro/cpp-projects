/*
   ...
   -lstdc++fs -static -std=c++17
    ...
    - TODO:
        ...
        - Catmull-Rom splines are a type of interpolating spline (the curve goes through the control
*/
...
struct Spline
{
    ...
    point2D getSplinePoint(float t, bool looped = false)
    {
      ...
      float tx =
          0.5 * (points[p0].x * q1 + points[p1].x * q2 + points[p2].x * q3 + points[p3].x * q4);
      float ty =
          0.5 * (points[p0].y * q1 + points[p1].y * q2 + points[p2].y * q3 + points[p3].y * q4);
...
class Splines : public olc::PixelGameEngine
{
  ...
  protected:
    virtual bool OnUserCreate() override
    {
      ...
      path.points = {{10, 41}, {20, 41}, {30, 41}, {40, 41}, {50, 41},
                     {60, 41}, {70, 41}, {80, 41}, {90, 41}, {100, 41}};
    ...
    bool OnUserUpdate(float fElapsedTime) override
    {
      ...
      FillRect(path.points[selectedPoint].x - 1, path.points[selectedPoint].y - 1, 3, 3,
               olc::YELLOW);
      DrawString(path.points[selectedPoint].x, path.points[selectedPoint].y,
                 std::to_string(selectedPoint), olc::WHITE);
      ...
      // offeset by 5 pixels
      DrawLine(5.0 * sin(r) + p1.x, 5.0 * cos(r) + p1.y, -5.0 * sin(r) + p1.x, -5.0 * cos(r) + p1.y,
               olc::BLUE);
      // TODO: the speed should be proportional to the gradient length
      return true;
    }
};
...
int main()
{
  ...
  // TODO: fix the draw string to work with small screens
