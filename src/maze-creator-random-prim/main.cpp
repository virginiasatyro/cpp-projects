...
class Mazes : public olc::PixelGameEngine
{
  ...
  protected:
    ...
    bool OnUserUpdate(float fElapsedTime) override
    {
      ...
      {
        // static list for frontier cells
        static std::vector<std::pair<int, int>> frontier;
        ...
        // while there is still frontier
        if (!frontier.empty())
        {
          // pick random frontier cell
          int i = rand() % frontier.size();
          auto [x, y] = frontier[i];

          // find visited neighbours
          std::vector<std::pair<int, int>> neighbours;

          if (y > 0 && (maze[(y - 1) * mazeWidth + x] & CELL_VISITED))
          {
            neighbours.emplace_back(x, y - 1);
          }
          if (x < mazeWidth - 1 && (maze[y * mazeWidth + (x + 1)] & CELL_VISITED))
          {
            neighbours.emplace_back(x + 1, y);
          }
          if (y < mazeHeight - 1 && (maze[(y + 1) * mazeWidth + x] & CELL_VISITED))
          {
            neighbours.emplace_back(x, y + 1);
          }
          if (x > 0 && (maze[y * mazeWidth + (x - 1)] & CELL_VISITED))
          {
            neighbours.emplace_back(x - 1, y);
          }

          // connect with random neighbour
          if (!neighbours.empty())
          {
            auto [nx, ny] = neighbours[rand() % neighbours.size()];

            if (nx == x && ny == y - 1)  // north
            {
              maze[y * mazeWidth + x] |= CELL_PATH_N | CELL_VISITED;
              maze[ny * mazeWidth + nx] |= CELL_PATH_S;
            }
            else if (nx == x + 1 && ny == y)  // east
            {
              maze[y * mazeWidth + x] |= CELL_PATH_E | CELL_VISITED;
              maze[ny * mazeWidth + nx] |= CELL_PATH_W;
            }
            else if (nx == x && ny == y + 1)  // south
            {
              maze[y * mazeWidth + x] |= CELL_PATH_S | CELL_VISITED;
              maze[ny * mazeWidth + nx] |= CELL_PATH_N;
            }
            else if (nx == x - 1 && ny == y)  // west
            {
              maze[y * mazeWidth + x] |= CELL_PATH_W | CELL_VISITED;
              maze[ny * mazeWidth + nx] |= CELL_PATH_E;
            }

            // mark frontier cell as visited
            visitedCells++;
          ...
          // Remove frontier cell from list
