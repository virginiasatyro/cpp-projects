...
class BubblePhysics : public olc::PixelGameEngine
{
  ...
  public:
    bool OnUserCreate() override
    {
      ...
      {
        modelBubble.push_back(
            {cosf(1 / (float)(points - 1) * 2.0 * pi), (float)(points - 1) * 2.0f * pi});
      }
      ...
      for (int i = 0; i < 10; i++)
      {
        // addBubble(rand() % ScreenWidth(), rand() % ScreenHeight(), defaultRadius); // not really
        // randon
    ...
    bool OnUserUpdate(float fElapsedTime) override
    {
      ...
      for (auto &bubble : vecBubbles)  // teste every bubble
      {
        for (auto &target : vecBubbles)  // teste against a target bubble
        {
          ...
          {
            if (doBubblesOverlap(bubble.px, bubble.py, bubble.radius, target.px, target.py,
                                 target.radius))
            {
              // collision occured
              ...
              float distance = sqrtf((bubble.px - target.px) * (bubble.px - target.px) +
                                     (bubble.py - target.py) * (bubble.py - target.py));
      ...
      for (auto c : vecCollidingPairs)
      {
        ...
        float distance =
            sqrtf((b1->px - b2->px) * (b1->px - b2->px) + (b1->py - b2->py) * (b1->py - b2->py));
        ...
        float m1 =
            (dpNorm1 * (b1->mass - b2->mass) + 2.0f * b2->mass * dpNorm2) / (b1->mass + b2->mass);
        float m2 =
            (dpNorm2 * (b2->mass - b1->mass) + 2.0f * b1->mass * dpNorm1) / (b1->mass + b2->mass);
