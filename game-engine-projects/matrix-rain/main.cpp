...
class MatrixRain : public olc::PixelGameEngine
{
  ...
  private:
    ...
    char randomCharacter()
    {
      ...
      // return (char)(rand() % 0x1EF + 0x00C0); // crazy characets - not very good too many spaces
      // - possible problem with my machine
    }
    ...
    bool OnUserUpdate(float fElapsedTime) override
    {
      ...
      for (auto &s : listStreamers)
      {
        ...
        for (int i = 0; i < s.text.size(); i++)
        {
          ...
          DrawString(s.column, 8 * ((int)s.position - i), std::string(1, s.text[charIndex]),
                     color);  // why 8*??
