  ...
  public:
    ...
    void drawSelf(olc::PixelGameEngine &pge, olc::Sprite *sprGFX, olc::vi2d screenOffset)
    {
      ...
      for (int32_t i = 0; i < visibleItems; i++)
      {
        ...
        // Display Item Header
        pge.DrawString(screenLocation, items[topLeftItem + i].name,
                       items[topLeftItem + i].enabled ? olc::WHITE : olc::DARK_GREY);
      ...
      // cursorPos.x = (cellCursor.x * (cellSize.x + cellPadding.x)) * patch + screenOffset.x -
      // patch; cursorPos.y = ((cellCursor.y - topVisibleRow) * (cellSize.y + cellPadding.y)) *
      // patch + screenOffset.y + patch;
