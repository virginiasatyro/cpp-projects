...
class DireWolf : public olc::PixelGameEngine
{
  ...
  protected:
    ...
    bool OnUserUpdate(float fElapsedTime) override
    {
      ...
      {
        DrawPartialSprite((ScreenWidth() / 2) - 16, (ScreenHeight() / 2) - 16, sprWolf, 0, 0, 45,
                          25);
      }
      else if (countTime > 0.14 && countTime <= 0.21)
      {
        DrawPartialSprite((ScreenWidth() / 2) - 16, (ScreenHeight() / 2) - 16, sprWolf, 45, 0, 45,
                          24);
      }
      else if (countTime > 0.21 && countTime <= 0.28)
      {
        DrawPartialSprite((ScreenWidth() / 2) - 16, (ScreenHeight() / 2) - 16, sprWolf, 90, 0, 45,
                          25);
      }
      else if (countTime > 0.28)
      {
        DrawPartialSprite((ScreenWidth() / 2) - 16, (ScreenHeight() / 2) - 16, sprWolf, 135, 0, 45,
                          24);
        countTime = 0;
      }
      else
